                             ...
                             Angle angle)
{
   ...
   *    Layout::Scale(1000). The latter would give a fractional scale
   *    that is not equal to the integer scaling factor used by
   *    DrawPolygon. The latter is inappropriate in this use-case.
   *    The arrow would not fill the square if we used the latter.
   *  - The scaling done by DrawPolygon gives us a reduction of precision.
   *    This is unfortunate but unavoidable with this interface.
   */
  ...
  const auto tail_width = 170 * size / scale;
  ...
  PolygonRotateShift(arrow, ARRAY_SIZE(arrow), pt.x, pt.y, angle);
